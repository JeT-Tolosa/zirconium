...
declare module 'jspanel4' {
  ...
  export interface IJSPanelInstance extends HTMLDivElement {
    ...
    addToolbar(
      ...
      toolbar: IJSPanelContentArray,
      callback?: IJSPanelCallback,
    ): this;
    close(
      callback?: (
        this: string | IJSPanelInstance,
        id: string,
    ...
    getChildpanels<TList = HTMLElement[]>(
      callback?: IJSPanelChildPanelsCallback,
    ): TList;
    isChildpanel(
      callback?: IJSPanelChildPanelCallback,
    ): IJSPanelInstance | false;
    ...
    overlaps(
      reference: 'window' | 'parent' | string,
      elmtBox: 'paddingbox' | string,
      event: PointerEvent,
    ): IJSPanelOverlaps;
    reposition(
  ...
  // Callbacks.
  export type IJSPanelAfterInsertCallback = (
  ...
  export type IJSPanelChildPanelCallback = (
    this: IJSPanelInstance,
    panel: IJSPanelInstance,
    parentpanel: IJSPanelInstance | null,
  ) => void;
  export type IJSPanelChildPanelsCallback<TList = HTMLElement[]> = (
    this: IJSPanelInstance,
    panel: IJSPanelInstance,
    index: number,
    list: TList,
  ...
  export type IJSPanelDragCallback = (
    this: IJSPanelInstance,
    panel: IJSPanelInstance,
    paneldata: IJSPanelData,
    event: PointerEvent,
  ) => void;
  export type IJSPanelDropCallback = (
    this: IJSPanelInstance,
    panel: IJSPanelInstance,
    target: HTMLElement,
    source: HTMLElement,
  ) => void;
  export type IJSPanelFetchCallback = (
    this: Response,
    response: Response,
    panel: IJSPanelInstance,
  ) => void;
  export type IJSPanelFunction<TReturn = any> = (...args: any[]) => TReturn;
  export type IJSPanelOnBeforeCloseCallback = (
    this: IJSPanelControl,
    panel: IJSPanelInstance,
    status: JSPanelStatus,
    closedByUser: boolean,
  ) => boolean;
  export type IJSPanelOnClosedCallback = (
    this: IJSPanelControl,
    panel: IJSPanelInstance,
    closedByUser: boolean,
  ) => boolean;
  export type IJSPanelParentCallback = (
    this: IJSPanelInstance,
    parent: IJSPanelInstance,
    size: IJSPanelSize,
  ) => void;
  export type IJSPanelPositionCallback<TReturn = number | string> = (
    this: IJSPanelPosition,
    pos: IJSPanelPosition,
    position: IJSPanelPositionOptions,
  ) => TReturn;
  export type IJSPanelResizeCallback = (
    this: IJSPanelInstance,
    panel: IJSPanelInstance,
    paneldata: IJSPanelData,
    event: PointerEvent,
  ...
  export type IJSPanelWindowCallback = (
    this: IJSPanelInstance,
    event: UIEvent,
    panel: IJSPanelInstance,
  ) => void;
  export type IJSPanelXhrCallback = (
    this: XMLHttpRequest,
    xhr: XMLHttpRequest,
    panel: IJSPanelInstance,
  ) => void;
