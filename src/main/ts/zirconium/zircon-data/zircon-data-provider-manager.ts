...
// this manager stores data provider instances
// wether they were generated by user states
// or directly generated by other objects
...
> extends ZirconAppObject<R> {
  ...
  private async onSTATE_SNAPSHOT_REGISTERED(
    ...
    if (
      this.getApplication()
        .getObjectManager()
        .isTypeOf(state.type, ZIRCON_DATA_PROVIDER_TYPE)
    ) {
      const providerState: ZirconDataProviderState =
        state as ZirconDataProviderState;
      if (providerState.active) {
        const obj: ZirconObject = await this.getApplication()
          .getObjectManager()
          .getInstance(state.id);
        if (!obj) {
          throw new Error(
            `registered object state of type data provider (${state.type}) cannot be created`,
          );
        }
        if (!(obj instanceof ZirconDataProvider)) {
          throw new Error(
            `registered object state of type data provider (${state.type}) creates an non data provider object ${obj.getType()}`,
          );
        }
        obj.activate(true);
      }
  ...
  public unregisterDataProvider(dataProviderId: string): boolean {
    ...
    // unconnect data provider to app event bus
