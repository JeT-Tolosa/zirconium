...
export class CollectionCatalogSelectorComponent {
  ...
  private __selectorLabel: HTMLElement = null;

  private __collectionSelect: HTMLElementTagNameMap['ui5-select'] | null = null;

  private __createNewCollectionButton:
    | HTMLElementTagNameMap['ui5-button']
    | null = null;

  private __collectionIdLabel: HTMLElement = null;

  private __catCountLabel: HTMLElement = null;
  ...
  private _onSelectedItemCollectionChange:
  ...
  public getMainDiv(): HTMLDivElement {
    ...
    this.__mainDiv.append(
      this.getSelectorLabel(),
      this.getItemCollectionSelect(),
      this.getCreateNewCatalogButton(),
      this.getCatIdLabel(),
      this.getCatCountLabel(),
    );

    return this.__mainDiv;
  }

  public getSelectorLabel(): HTMLElement {
    if (this.__selectorLabel) {
      return this.__selectorLabel;
    }

    this.__selectorLabel = document.createElement('ui5-label');

    this.__selectorLabel.textContent = 'Choose catalog:';

    return this.__selectorLabel;
  }

  public getCatIdLabel(): HTMLElement {
    if (this.__collectionIdLabel) {
      return this.__collectionIdLabel;
    }

    this.__collectionIdLabel = document.createElement('ui5-label');

    this.__collectionIdLabel.textContent = 'no selected';

    return this.__collectionIdLabel;
  }

  public getCatCountLabel(): HTMLElement {
    if (this.__catCountLabel) {
      return this.__catCountLabel;
    }

    this.__catCountLabel = document.createElement('ui5-label');

    this.__catCountLabel.textContent = '?';
  ...
  public getCreateNewCatalogButton(): HTMLElement {
    ...
    this.__createNewCollectionButton = document.createElement('ui5-button');

    this.__createNewCollectionButton.textContent = 'Create';

    this.__createNewCollectionButton.setAttribute('icon', 'add');

    this.__createNewCollectionButton.addEventListener('click', async () => {
  ...
  public getItemCollectionSelect(): HTMLElementTagNameMap['ui5-select'] {
    if (this.__collectionSelect) {
      return this.__collectionSelect;
    }

    this.__collectionSelect = document.createElement('ui5-select');

    this.__collectionSelect.id = uuid();

    this.__collectionSelect.addEventListener('change', (ev: Event) => {
      const select = ev.target as HTMLElementTagNameMap['ui5-select'];

      const option = select.selectedOption;

      if (option) {
        this.selectItemCollection(option.value);
      }
    });
  ...
  public getSelectedItemCollectionId(): string {
    const option = this.__collectionSelect?.selectedOption;

    return option?.value ?? null;
  }

  public selectItemCollection(collectionId: string): boolean {
    const select = this.getItemCollectionSelect();

    const options = Array.from(
      select.children,
    ) as HTMLElementTagNameMap['ui5-option'][];

    const index = options.findIndex((option) => option.value === collectionId);
    ...
    this.getCatIdLabel().textContent = `ID = ${collectionId} index = ${index}`;
  ...
  public updateOptions(): void {
    const previousSelectedCatalogId = this.getSelectedItemCollectionId();
    ...
    this.__itemCollectionDescriptors.forEach(
      (descriptor: ItemCollectionDescriptor) => {
    ...
    if (shouldRestoreSelection) {
      select.value = previousSelectedCatalogId;
    }

    // Sinon sélectionner automatiquement la première collection
    else if (this.__itemCollectionDescriptors.length > 0) {
      const firstId = this.__itemCollectionDescriptors[0].id;

      select.value = firstId;

      this._onSelectedItemCollectionChange?.(firstId);

      this.getCatIdLabel().textContent = `ID = ${firstId} index = 0`;
    }
  }
}
...
//   private __createNewCollectionButton: Button = null;
